...
function initSlider() {
    ...
    function moveSlide(nextIndex, direction) {
        if (isAnimating) return;
        isAnimating = true;
        resetAutoSlide(); // Reset timer saat slide manual

        // Siapkan slide yang akan digeser
        const nextSlide = slides[nextIndex];
        const currentSlideElement = slides[currentSlide];
        const prevSlide = slides[(currentSlide - 1 + slides.length) % slides.length];
        const nextAdjSlide = slides[(currentSlide + 1) % slides.length];
    ...
    function moveSlideComplex(targetIndex) {
        ...
        const distance = Math.abs(targetIndex - currentSlide);
        if (distance > 1) {
            const direction = targetIndex > currentSlide ? 'next' : 'prev';
            const midIndex = currentSlide + (direction === 'next' ? 1 : -1);

            // Animasi pertama
            moveSlide(midIndex, direction);

            // Animasi kedua setelah yang pertama selesai
            setTimeout(() => {
                moveSlide(targetIndex, direction);
            }, 600);
        } else {
            moveSlide(targetIndex, targetIndex > currentSlide ? 'next' : 'prev');
        }
    }
